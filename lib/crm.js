...
// Agrega una nota al chatter de una oportunidad ya existente (1 RPC en vez de repetir el pushLead).
// La usa el diagnóstico para enriquecer un lead que acaba de crear.
async function odooAddLeadNote(c, leadId, note) {
  await odooExec(c, "crm.lead", "message_post", [[leadId]], { body: String(note).slice(0, 8000) });
  return { ok: true, driver: "odoo", id: leadId };
}

// Manda un correo AL PROSPECTO desde Odoo (mail.mail → addon minka_ses → API de AWS SES) y lo deja
// registrado en el chatter de la oportunidad.
async function odooSendLeadEmail(c, leadId, { to, subject, html, from = "", replyTo = "" } = {}) {
  ...
  const mailId = await odooExec(c, "mail.mail", "create", [vals]);
  ...
  await odooExec(c, "mail.mail", "send", [[mailId]], { raise_exception: true });
  // Nota en el chatter DESPUÉS del envío: si el send falla, esta línea no corre y el historial del
  // lead nunca afirma un envío que no ocurrió.
  if (leadId) {
    await odooExec(c, "crm.lead", "message_post", [[leadId]],
...
// Agrega una nota al chatter de una oportunidad existente. Sólo driver odoo; nunca lanza.
async function addLeadNote(leadId, note, opts = {}) {
  let d = "none";
  if (!leadId || !note) return { ok: false, driver: d, detail: "leadId y note requeridos" };
  try {
    d = driver(opts.tenant);
    if (d === "odoo") return await odooAddLeadNote(credsFor(opts.tenant), leadId, note);
...
// Manda un correo al prospecto desde Odoo. Sólo driver odoo; nunca lanza.
async function sendLeadEmail(leadId, mail, opts = {}) {
  let d = "none";
  try {
    d = driver(opts.tenant);
    if (d === "odoo") return await odooSendLeadEmail(credsFor(opts.tenant), leadId, mail);
...
module.exports = {
  pushLead, findByEmail, attachToLead, addLeadNote, sendLeadEmail, noteOnce, tagLead, driver,
