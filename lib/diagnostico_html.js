...
function telegramUrl(p, report) {
  try {
    const plan = PLAN_CODE[report?.plan?.slug] || "R";
    ...
    if (!/^[A-Za-z0-9_-]{1,64}$/.test(payload)) return null; // no cupo o quedó vacío: sin atajo
...
module.exports = { renderDiagnosticoHTML, renderDiagnosticoEmail, slugify, activarUrl, telegramUrl, firstName, PLAN_LABEL };
