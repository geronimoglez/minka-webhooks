...
function flashFor(q) {
  ...
  const M = {
    ...
    "d:parcial": `Alcanzamos a guardar ${n} ${n === 1 ? "documento" : "documentos"}; los demás no. ` +
      "Vuelve a subir los que falten, o mándalos por WhatsApp.",
...
// `paid` NO sale de la query string: viene del token firmado (ver lib/sign.js). Si dependiera de
// `?p=ok`, cualquiera podría escribirlo a mano y la página diría "Lugar apartado" sin que exista el
// cobro. Una pantalla que miente sobre un pago es peor que no tenerla: alguien la va a usar como
// comprobante. Ship-review 2026-07-30.
function graciasPage({ token, cfg, query = {}, paid = false }) {
  // El mensaje de "recibimos tu apartado" también se calla si el token no lo respalda.
  const flash = flashFor(query.p === "ok" && !paid ? { ...query, p: "" } : query);
...
${footer(cfg)}`;
  ...
  // Sin pasarela configurada no se pinta un botón que llevaría a una página de error: se dice el
  // monto y se aparta por WhatsApp. El embudo sigue capturando postulaciones igual.
  const apartadoSinPasarela = `<div class="card">
  ...
  Escríbenos por WhatsApp y te pasamos los datos para apartarlo.</p>
  <a class="btn" style="margin-top:22px" href="https://wa.me/${esc(cfg.whatsapp)}?text=${waTexto}"
     target="_blank" rel="noopener">Apartar por WhatsApp</a>
</div>`;
...
</div>` : !cfg.pagoEnLinea ? apartadoSinPasarela : `<div class="card">
  <span class="eyebrow">Aparta tu lugar</span>
  <h2>Son ${cfg.lugares} lugares.</h2>
  <div class="price">${money(cfg.apartadoMxn)}</div>
  <p>Se abonan al precio de tu pase. Si decides no asistir, no se reembolsan.
