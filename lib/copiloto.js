...
/**
 * El prompt se COMPONE con la voz del tenant. Lo que va fijo es la doctrina de hilo —cómo se
 * responde en público— que es de la clase de negocio, no de una marca. Lo que cambia es quién
 * escribe, qué vende y qué no puede decir.
 *
 * Sin esto, un borrador para la cuenta de un cliente saldría con la voz de Gerónimo y
 * mencionando a Minka. El dueño dejaría de aprobar, y el servicio se muere ahí.
 */
function sistemaPara(tenant = {}) {
  const t = require("./tenants").MINKA;
  const handle = tenant.handle || t.handle;
  const negocio = tenant.negocio || t.negocio;
  const voz = tenant.voz || t.voz;
  const propias = (tenant.reglas || []).map((r) => `- ${r}`).join("\n");

  return `Eres el copiloto de comentarios de @${handle}.
EL NEGOCIO: ${negocio}
${voz}
 ...
 */
async function analizar(texto, contextoPieza, tenant) {
...
COMENTARIO: ${String(texto).slice(0, 800)}`;
  ...
  // Techo duro: el webhook AHORA espera al clasificador antes de contestarle a Meta, y Meta da
  // ~20 s antes de dar la entrega por fallida y reintentar. Un modelo colgado no puede
  // arrastrar al endpoint: a los 12 s se aborta y se degrada a manual — el camino seguro que
  // ya existe (sin borrador → lo lee un humano).
  const corte = AbortSignal.timeout(Number(process.env.IG_COPILOTO_TIMEOUT_MS) || 12000);
  ...
  try {
    const r = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: MODELO,
        messages: [{ role: "system", content: sistemaPara(tenant) }, { role: "user", content: user }],
        temperature: 0.6,
        max_tokens: 400,
...
module.exports = { ARQUETIPOS, analizar, decidir, sistemaPara, MODELO };
